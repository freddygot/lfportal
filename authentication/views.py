...
def dashboard_view(request):
    ...
    if profile.role == 'franchise_taker':
        return franchise_dashboard_view(request, user)
    elif profile.role == 'employee':
        return employee_dashboard_view(request, user)
    else:
        return render(request, 'authentication/dashboard.html', {'error': 'Role not recognized.'})

@login_required
def franchise_dashboard_view(request, user):
    profile = user.profile
...
@login_required
def employee_dashboard_view(request, user):
    profile = user.profile
